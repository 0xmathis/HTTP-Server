...
int detect_quoted_string(Node *parent, const char *ptr) {
    ...
    if (*ptr == '"') {
        ...
        while (detect_qdtext(quoted_stringNode, ptr) == 0 || detect_qdtext(quoted_stringNode, ptr) == 0) {
            ptr += getLength(getLastChild(quoted_stringNode));
        }
...
int detect_HEXDIGIT(Node *parent, const char *ptr) {
    Node *HEXDIGITNode = newChild(parent);
    initNode(HEXDIGITNode, "HEXDIGIT", ptr, 0);

    if (detect_DIGIT(HEXDIGITNode, ptr) == 0) {
        ptr += getLength(getLastChild(HEXDIGITNode));
    } else if (strchr("ABCDEF", *ptr) != NULL){
        initNode(newChild(sub_delimsNode), "case_insensitive_string", ptr, 1);
        ptr += getLength(getLastChild(sub_delimsNode));
    } else if (strchr("abcdef", *ptr) != NULL){
        initNode(newChild(sub_delimsNode), "case_insensitive_string", ptr, 1);
    ...
    setLength(HEXDIGITNode, getSumLengthChildren(HEXDIGITNode));
...
int detect_qdtext(Node *parent, const char *ptr) {
    ...
    if (detect_HTAB(qdtextNode, ptr) == 0) {
        ...
        ptr += getLength(getLastChild(qdtextNode));
    } else if (strchr("#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[", *ptr) != NULL){
        initNode(newChild(qdtextNode), "case_insensitive_string", ptr, 1);
        ptr += getLength(getLastChild(qdtextNode));
    } else if (strchr("]^_`abcdefghijklmnopqrstuvwxyz{|}~", *ptr) != NULL){
        initNode(newChild(qdtextNode), "case_insensitive_string", ptr, 1);
        ptr += getLength(getLastChild(qdtextNode));
